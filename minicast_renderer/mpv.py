...
from minicast.utils import Setting
from minicast.renderer import Renderer, RendererSetting
from minicast.gui import App, MenuItem
...
class MPVRenderer(Renderer):
    ...
    def __init__(self, lang=gettext.gettext, path="mpv"):
        ...
        if os.name == 'nt':
            # Windows 命名管道必须是 \\.\pipe\<name> 这种绝对路径，
            # 不能用 get_base_path / os.path.join 拼接 —— 那会把开头的
            # "\\" 当成驱动器相对路径，破坏成 "C:\.\pipe\..."，导致 mpv
            # 创建不出管道、IPC 永远连不上，表现为投屏后播放器无反应。
            self.mpv_sock = r"\\.\pipe\minicast_mpvsocket{}".format(mpv_rand)
        else:
            self.mpv_sock = '/tmp/minicast_mpvsocket{}'.format(mpv_rand)
        self.path = path
        self.proc = None
    ...
    def set_media_url(self, url, start="0"):
        """ data : string
        """
        # mpv 0.41 及以后不再支持 loadfile 的第四个 options 参数
        # （传入 "start=0,fullscreen=yes" 之类会被拒绝为 invalid parameter，
        # 导致投屏后 mpv 毫无反应）。改为先 loadfile，再用独立命令设置选项。
        self.send_command(['loadfile', url, 'replace'])
        if start and start != "0":
            self.send_command(['set_property', 'start', start])
    ...
    def start_ipc(self):
        ...
        while self.ipc_running and self.running and self.mpv_thread.is_alive():
            try:
                ...
                logger.debug("mpv ipc socket reconnecting: %s", e)
                continue
            res = b''
            msgs = None
    ...
    def start_mpv(self):
        ...
        if error_time <= 0:
            ...
            cherrypy.engine.publish("app_notify", "MiniCast", "MPV Can't start")
