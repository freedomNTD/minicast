...
class App:
    ...
    def append_menu_item_after(self, id, menu_item):
        if self.platform == Platform.Darwin:
            ...
            logger.debug("insert_menu_item_after index: %s", index)
            if index != -1:
    ...
    def alert(self, content):
        if self.platform == Platform.Darwin:
            ...
            self.notification(content, "MiniCast")
    ...
    def dialog(self, content, callback=None, cancel="Cancel", ok="Ok"):
        if self.platform == Platform.Darwin:
            ...
            self.notification("MiniCast", content)
            if callback:
                callback()
...
if __name__ == '__main__':
    class DemoApp(App):
        def __init__(self):
            super(DemoApp, self).__init__("MiniCast",
