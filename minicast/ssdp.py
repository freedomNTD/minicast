...
# Based on public UPnP/SSDP reference implementations.
#
...
class SSDPServer:
    ...
    def run(self):
        ...
        for ip, mask in self.ip_list:
            try:
                logger.info('add membership %s', ip)
        ...
        try:
            ...
            logger.error(e)
            cherrypy.engine.publish("app_notify", "MiniCast", "SSDP Can't start")
        ...
        self.shutdown()
        for ip, mask in self.ip_list:
            logger.info("drop membership %s", ip)
            mreq = socket.inet_aton(SSDP_ADDR) + socket.inet_aton(ip)
            try:
        ...
        try:
            ...
            if self.sock:
                try:
                    ...
                    logger.warning("error sending byebye notification: %r", e)
        except KeyError as e:
            logger.warning("error building byebye notification: %r", e)
