...
class Service:
    ...
    def __init__(self, name, namespace='', actions=None):
        self.name = name
...
class DLNAProtocol(Protocol):
    ...
    def call(self, rawbody):
        ...
        through the DLNAHandler (see class DLNAHandler in this module).
    ...
    def AVTransport_SetAVTransportURI(self, data):
        ...
        metadata = data['CurrentURIMetaData'].value
        # metadata 为空（客户端未提供或为空字符串）属于合法情况，
        # 不应报错。仅当确有内容时才尝试解析标题。
        if metadata:
            try:
                meta = etree.fromstring(metadata.encode())
                ...
                metadata = etree.tostring(meta, encoding="UTF-8",
                                          xml_declaration=False).decode()
            except Exception as e:
                logger.debug("parse CurrentURIMetaData failed: %s", e)
        self.set_state('CurrentTrackMetaData', metadata if metadata else '')
...
class Handler:
    ...
    def GET(self, param=None, *args, **kwargs):
        ...
        # MiniCast 不再提供网页设置页 / 插件管理 API。
        # 非投屏相关的根路径请求返回简单提示。
        if param is not None:
            raise cherrypy.HTTPRedirect('/')
        cherrypy.response.headers['Content-Type'] = 'text/html; charset=utf-8'
        return b'<!DOCTYPE html><html><head><meta charset="utf-8"><title>MiniCast</title></head>' \
               b'<body><h2>MiniCast is running.</h2>' \
               b'<p>DLNA renderer ready. Cast from your phone / DLNA client.</p></body></html>'

    def POST(self, *args, **kwargs):
        cherrypy.response.headers['Content-Type'] = 'application/json;charset:utf-8'
        return json.dumps({'code': 0, 'message': 'MiniCast web UI disabled.'}, indent=4).encode()
...
class DLNAHandler(Handler):
    ...
    def build_description(self):
        self.description = load_xml(XMLPath.DESCRIPTION.value).format(
            ...
            manufacturer="MiniCast",
            manufacturer_url="https://github.com/",
    ...
    def SUBSCRIBE(self, service="", param=""):
        ...
        if param == 'event':
            ...
            if SID:
                logger.debug("RENEW SUBSCRIBE: %s", service)
                ...
                logger.debug("ADD SUBSCRIBE: %s", service)
    ...
    def UNSUBSCRIBE(self, service, param):
        ...
        if param == 'event':
            SID = cherrypy.request.headers.get('SID')
            if SID:
                logger.debug("REMOVE SUBSCRIBE: %s", service)
