...
class Service:
    ...
    def __init__(self, renderer, protocol):
        ...
        cherrypy.config.update({
            'log.screen': False,
            'log.access_file': os.path.join(SETTING_DIR, 'minicast.log'),
            'log.error_file': os.path.join(SETTING_DIR, 'minicast.log'),
        })
    ...
    def run(self):
        ...
        if port != Setting.get(SettingProperty.ApplicationPort, 0):
            ...
            logger.info("Change usn to: %s", usn)
            Setting.set(SettingProperty.ApplicationPort, port)
            name = "MiniCast({0:04d})".format(random.randint(0, 9999))
            logger.info("Change name to: %s", name)
